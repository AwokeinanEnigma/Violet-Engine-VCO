...
namespace VCO.Data
{
    internal class SaveFileManager
    {
        ...
        {
            bool flag = true;
            flag &= saveTag.TryGet<NbtInt>("idx", out NbtInt nbtInt);
            flag &= saveTag.TryGet<NbtIntArray>("prty", out NbtIntArray nbtIntArray);
            flag &= saveTag.TryGet<NbtString>("map", out NbtString nbtString);
            flag &= saveTag.TryGet<NbtInt>("x", out NbtInt nbtInt2);
            flag &= saveTag.TryGet<NbtInt>("y", out NbtInt nbtInt3);
            flag &= saveTag.TryGet<NbtInt>("tm", out NbtInt nbtInt4);
            flag &= saveTag.TryGet<NbtInt>("flv", out NbtInt nbtInt5);
            Vector2f position = default(Vector2f);
            if (nbtInt2 != null && nbtInt3 != null)
            {
                position.X = nbtInt2.IntValue;
                position.Y = nbtInt3.IntValue;
            }
        ...
        public void LoadFile(int saveIndex)
        {
            ...
            if (nbtList != null)
            {
                ...
                {
                    NbtCompound nbtCompound = (NbtCompound)nbtTag;
                    bool flag = nbtCompound.TryGet<NbtCompound>("prf", out NbtCompound saveTag);
                    if (flag)
                    {
                        ...
                        if (saveProfile.Index == saveIndex)
                        {
                            ...
                            flag &= nbtCompound.TryGet<NbtIntArray>("flags", out NbtIntArray flagTag);
                            FlagManager.Instance.LoadFromNBT(flagTag);
                            flag &= nbtCompound.TryGet<NbtIntArray>("vals", out NbtIntArray valueTag);
                            ValueManager.Instance.LoadFromNBT(valueTag);
                            flag &= nbtCompound.TryGet<NbtList>("names", out NbtList nameListTag);
