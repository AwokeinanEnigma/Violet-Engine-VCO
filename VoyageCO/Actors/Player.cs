    ...
    internal class Player : SolidActor
    {
        ...
        public override bool MovementLocked
        {
            ...
            set
            {
                ...
                this.playerGraphic.SpeedModifier = this.isMovementLocked ? 0 : 1;
        ...
        private void HandleCornerSliding()
        {
            if (this.direction % 2 == 0)
            {
                ...
                for (int i = num; i > 0; i--)
                {
                    bool flag = this.collisionManager.PlaceFree(this, this.position + vector2f + vector2f2 * i, null, this.ignoreCollisionTypes);
                    if (flag)
                    {
                        num2 = i;
                        break;
                ...
                for (int j = num; j > 0; j--)
                {
                    bool flag2 = this.collisionManager.PlaceFree(this, this.position + vector2f - vector2f2 * j, null, this.ignoreCollisionTypes);
                    if (flag2)
                    {
                        num3 = j;
                        break;
        ...
        public override void Update()
        {
            if (this.mover != null)
            {
                ...
                this.speed = (int)VectorMath.Magnitude(this.velocity);
                this.animator.UpdateSubsprite(this.GetAnimationContext());
