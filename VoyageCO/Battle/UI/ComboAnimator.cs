...
namespace VCO.Battle.UI
{
    internal class ComboAnimator : IDisposable
    {
        ...
        public void Setup(Graphic graphic, PlayerCombatant playa)
        {
            ...
            this.bounce = 0f;
            this.bounceSpeed = 0f;
            this.position = this.enemyGraphic.Position - this.enemyGraphic.Origin + new Vector2f(enemyGraphic.TextureRect.Width / 2f, enemyGraphic.TextureRect.Height / 6f);
            this.position.X = (int)this.position.X;
            this.position.Y = (int)this.position.Y;
            this.size = new Vector2f(enemyGraphic.TextureRect.Width * 0.6f, 2f);
            ...
            Vector2f v = new Vector2f(0f, enemyGraphic.TextureRect.Height * 0.9f);
        ...
        public void AddHit(int damage, bool smash)
        {
            ...
            Vector2f vector2f = this.position - this.size / 2f + new Vector2f((int)(Engine.Random.NextDouble() * size.X), (int)(Engine.Random.NextDouble() * size.Y));
            ...
            Vector2f vector2f2 = new Vector2f(vector2f.X, this.enemyGraphic.Position.Y - this.enemyGraphic.Origin.Y + (int)(Engine.Random.NextDouble() * enemyGraphic.TextureRect.Height));
        ...
        private void AddStar()
        {
            if (this.starCount < 16)
            {
                ...
                this.starCount++;
                this.rotAngle -= (float)(6.283185307179586 / starCount);
                this.modAngle -= (float)(6.283185307179586 / starCount) / 2f;
            }
        }
        ...
        public void Update()
        {
            ...
            else if (this.state == ComboAnimator.State.Falling)
            {
                ...
                for (int j = 0; j < this.starCount; j++)
                {
                    ...
                    bool flag2 = this.starGraphics[j].Position.X < starGraphics[j].TextureRect.Width || this.starGraphics[j].Position.Y < starGraphics[j].TextureRect.Height || this.starGraphics[j].Position.X > 320L + starGraphics[j].TextureRect.Width || this.starGraphics[j].Position.Y > 180L + starGraphics[j].TextureRect.Height;
                    flag = (flag && flag2);
                }
            ...
            if (this.enemyGraphic != null && this.wobbleSpeed > 0f)
            {
                ...
                this.enemyGraphic.Rotation = (float)(Math.Sin(wobble) * wobbleDamp);
            }
