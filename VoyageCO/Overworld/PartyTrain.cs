...
namespace VCO.Overworld
{
    internal class PartyTrain
    {
        ...
        public void Reset(Vector2f position, int direction, TerrainType terrain, bool extend)
        {
            ...
            {
                this.recordPoints[i].position = ((!extend) ? position : (position + (this.recordPoints.Length - i) * v));
        ...
        public void Add(PartyFollower follower)
        {
            ...
            {
                float num3 = (i - num) / (float)(num2 - num);
                int num4 = i % this.recordPoints.Length;
                this.recordPoints[num4].position.X = (int)(position2.X + (position.X - position2.X) * num3);
                this.recordPoints[num4].position.Y = (int)(position2.Y + (position.Y - position2.Y) * num3);
        ...
        public PartyFollower Remove(PartyFollower follower)
        {
            ...
            for (int i = num; i < this.followers.Count; i++)
            {
                ...
                for (int j = num4; j <= num5; j++)
                {
                    ...
                    float num6 = (j - num4) / (float)(num5 - num4);
                    int place3 = num2 + (int)((place2 - num2) * num6);
