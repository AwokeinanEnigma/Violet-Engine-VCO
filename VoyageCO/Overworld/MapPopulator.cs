...
namespace VCO.Overworld
{
    internal class MapPopulator
    {
        public static List<NPC> GenerateNPCs(RenderPipeline pipeline, CollisionManager collisions, Map map)
        {
            ...
            {
                if (npcData.Enabled == FlagManager.Instance[npcData.Flag])
                {
                    object obj = null;
        ...
        public static List<EnemySpawner> GenerateSpawners(Map map)
        {
            ...
            foreach (Map.EnemySpawn enemySpawn in map.Spawns)
            {
                ...
                EnemySpawner item = new EnemySpawner(new FloatRect(enemySpawn.X, enemySpawn.Y, enemySpawn.Width, enemySpawn.Height), enemySpawn.Enemies);
                list.Add(item);
            }
            return list;
        }
