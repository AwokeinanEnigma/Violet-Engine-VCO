...
namespace VCO.GUI.NamingMenu
{
    internal class TextInputPanel : MenuPanel
    {
        ...
        private void CreateButtons()
        {
            ...
            {
                new TextRegion(new Vector2f(8f, this.size.Y - Fonts.Main.LineHeight), 1, Fonts.Main, this.inputTitles[(this.selectedPage + 1) % this.inputTitles.Count]),
                new TextRegion(new Vector2f(83f, this.size.Y - Fonts.Main.LineHeight), 1, Fonts.Main, StringFile.Instance.Get("naming.space").Value),
                new TextRegion(new Vector2f(this.size.X - 8f - 115f, this.size.Y - Fonts.Main.LineHeight), 1, Fonts.Main, StringFile.Instance.Get("naming.dontCare").Value),
                new TextRegion(new Vector2f(this.size.X - 8f - 50f, this.size.Y - Fonts.Main.LineHeight), 1, Fonts.Main, StringFile.Instance.Get("naming.back").Value),
                new TextRegion(new Vector2f(this.size.X - 8f - 10f, this.size.Y - Fonts.Main.LineHeight), 1, Fonts.Main, StringFile.Instance.Get("naming.ok").Value)
            };
        ...
        public override void AxisPressed(Vector2f axis)
        {
            if (this.selectingLetters)
            {
                ...
                {
                    float num = cursorX / (float)(this.inputChars[this.selectedPage][this.cursorY].Length - 1);
                    ...
                    this.cursorX = (this.selectingLetters ? ((int)Math.Round(num * (this.inputChars[this.selectedPage][this.cursorY].Length - 1))) : ((int)Math.Round(num * (this.buttonTexts.Length - 1))));
                }
            }
            else if (axis.X != 0f)
            {
            ...
            else if (axis.Y != 0f)
            {
                ...
                float num2 = cursorX / (float)(this.buttonTexts.Length - 1);
                this.cursorX = (int)Math.Round(num2 * (this.inputChars[this.selectedPage][this.cursorY].Length - 1));
                this.sfxCursorY.Play();
            }
            this.UpdateCursorPosition();
        }
